...
| `self_loop_directed.cypher` | `MATCH (p:Person)-[r:KNOWS]->(q:Person) RETURN r` | self-loop admits normally (both endpoints Person, edge Person→KNOWS→Person) — §4.5.2 self-loop witness (schema-level: source label == target label, two distinct `VarEndpoint`s) |
| `self_loop_same_binding.cypher` | `MATCH (p:Person)-[r:KNOWS]->(p) RETURN r` | same-binding self-loop: a single `VarEndpoint` names `p` on both sides. Locks §4.5.2's `touchingSide` "both sides are `n`" branch and the labelled Phase A2 self-loop case — resolves to the same singleton `ResolvedEdge{Person→KNOWS→Person}` as `self_loop_directed`, but through the both-sides-share-one-VarEndpoint code path. |
...
- `self_loop_directed` — §4.5.2's self-loop witness (§4.5.2 covers
  ...
  loop case in Phase A2). Two distinct `VarEndpoint`s (`p` and `q`)
  both labelled `Person`; the self-loop-ness is schema-level
  (`k.Source == k.Target`), not binding-level.
- `self_loop_same_binding` — §4.5.2's "both sides are `n`" branch:
  a single `VarEndpoint` names `p` on both endpoints of the same
  edge. Locks `touchingSide`'s both-sides return and the labelled
  Phase A2 self-loop path where the candidate cross-product
  degenerates to `{k.Source, k.Target} = {Person, Person}`. Same
...
- `unknown_property_union_type_differs.cypher`:
  ...
  `AUTHORED` and `LIKES` both declare a property `weight`, but as
  `INT NOT NULL` on `AUTHORED` and `FLOAT NOT NULL` on `LIKES`).
  The fixture: `MATCH (p:Person)-[r:AUTHORED|LIKES]->(post:Post)
  RETURN r.weight` → §4.8 step 2 fails → `ErrUnknownProperty` with
  the "type differs" prose. Pairing recorded in
  `invalid/schema.mapping.json`. (Gloss: the disagreeing-property
  name is `weight`, not `count` — `COUNT` is a GQL reserved word
  per `GQL.g4:3319`, so using it as a property identifier would
  fail at parse time before reaching §4.8.)
- `untyped_edge.cypher`:
