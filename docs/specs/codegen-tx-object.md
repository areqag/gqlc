...
| Begin refusal order (AGE) | swap the `pgx.Tx` check after the capability assertion | **MEASURED NO-OP**, declared as such in advance and against this table's own earlier prediction of a kill. 2026-08-24, run `32693045598`: `tx: Begin inside a transaction is refused` PASSED on the AGE arm with the two swapped. `pgx.Tx` satisfies the capability assertion and falls through to the check, so the refusal still fires. The prediction this row used to carry — "gets a savepoint tx" — was false, and it had been copied into the emitted comment in every generated AGE `db.go`; both are corrected. The order that *is* load-bearing is check-before-`b.Begin`, and that one has a row of its own below. |
| Begin refusal order, check before `b.Begin` (AGE) | move the `pgx.Tx` check below `b.Begin` | **MEASURED KILLED**, 2026-08-29, local Docker run on `test/gqlc-g8rc-age-savepoint-leak-witness`. `tx: the refused nested Begin opens no savepoint` FAILS at `live_test.go:915`: the refusal is handed back over a transaction now holding an abandoned `sp_1`. The other twelve AGE rows pass — `tx: Begin inside a transaction is refused` among them, which asserts only `require.Error` and so stayed green over the leak. That is why this row exists (bead `gqlc-g8rc`), and the kill is scoped to it. The scenario carries its own control: `serveNestedBegin` opens a savepoint through pgx's own nested `Begin` and requires the same `RELEASE SAVEPOINT sp_1` to succeed, so a probe gone blind to a renamed savepoint reads red rather than reading absence. |
