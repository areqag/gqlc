...
| `ErrOutOfC6Scope` | A parameter whose resolved type is not `ResolvedProperty`. Post-v1. The gate is a type assertion rather than a switch, so what reaches it is everything else satisfying `resolver.ResolvedType`: the other seven variants in value form — whole node, whole edge, edge union, scalar literal, list, temporal expression, unknown — and, on exactly the argument the column row below makes, their pointer forms, structs embedding either, and the nil interface. `TestUnmatchedResolvedTypeRefusesRatherThanFaulting/parameter` measures the four faulting shapes at this site from outside the package; the column row carries the argument and the limit. | Phase A |
