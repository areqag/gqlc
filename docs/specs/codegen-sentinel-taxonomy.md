...
| `ErrUnimplementedTypeKind` | A property type of a KIND gqlc has built no emission for — today `RECORD` and the closed dynamic unions, which the schema front end resolves and no backend renders. The one sentinel here carrying a "yet", and the only one that is not a target's answer: the two above it are refusals a backend makes and another backend may not, while this one holds on every target at once, which is why the refusal lives in `prepare.go` and both type maps are untouched by it. Apart from `ErrUnrepresentableWidth` because a table falling off its switch would name the wrong edit — there is no declared width to change, at any width, on any backend. Asked before the carrier question and asked RECURSIVELY through list elements, since a shallow check admits `LIST<RECORD<…>>` and lets its record die inside the table as a width error, which is the confusion the sentinel exists to prevent. | ADR 0039, `gqlc-h9n.33` |
...
| `ErrUnimplementedTypeKind` | A schema property whose type is of a kind gqlc emits nothing for, or whose type is a list reaching such a kind through its elements. Asked BEFORE both questions above it: a record has no carrier on any target, so the table would answer `ErrUnrepresentableWidth` and send the reader to change a width that is not the obstacle. The list recursion is what makes `LIST<RECORD<a INT32>>` land here rather than there. | Phase Z |
...
| `ErrUnimplementedTypeKind` | A projected column whose property type is of a kind gqlc emits nothing for. Assembled, one step past the width row's argument: Phase Z refuses a declared record property, so a column reaching this check is one no declared property backs. | Phase A |
