...
`ErrOutOfC2Scope` because that is the sentinel C2 shipped, three renames
...
sentinels `Generate` may return on a user's input. Every member is
paired with at least one negative fixture in `test/data/codegen/invalid`,
which `TestSentinelReachability` enforces.
...
| `ErrUnrepresentableWidth` | A projected column, or a query parameter, whose property width the target's type table has no carrier for. | Phase A |
| `ErrOutOfC6Scope` | A column referencing a node or edge type the schema does not declare. | Phase A |
...
| `ErrOutOfC6Scope` | An edge union with fewer than two candidates, or one naming a candidate the schema does not declare. | Phase A |
...
| `ErrOutOfC6Scope` | A list element chain whose leaf references an undeclared type, or whose leaf is a resolved type with no arm. | Phase A |
...
| `ErrAliasRequired` | A column whose text yields no row-field name at derivation. | Phase B |
...
## 3. Declared and deliberately unreachable
...
## 4. Adding, renaming or retiring a sentinel
...
1. Declare the sentinel in `internal/codegen/errors.go` with a doc
   comment saying what it refuses.
2. Add it to `allSentinels` if a user's input can reach it, and add a row
   to §1 above. Otherwise leave it out of the slice and add a row to §3
   saying why.
3. Add at least one row to §2 for each construct that routes to it —
   `TestSentinelReachability` needs a negative fixture under
   `test/data/codegen/invalid` for the same reason, and the fixture and
   the row describe the same construct.
4. A rename is the same three edits with the old name removed; a
   retirement drops the row, the slice entry and the fixture together.
