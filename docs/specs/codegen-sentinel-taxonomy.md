...
| `UUID` | `scopePackage` | `neo4j-go-v5`, `neo4j-go-v6` | `neo4j-go-v5`, `neo4j-go-v6` |
...
Six and three count the target axis alone. The batch axis moves
...
five more again on one of the second — and either neo4j major gains the
`UUID` row on one of the third, which `apache-age-pgx-v5` already
counts. Fourteen for a batch of all three shapes under either neo4j major,
ten for an AGE-only one. `NODE TYPE DBTX` is refused
on a name neo4j leaves free, and `NODE TYPE UUID` on one that
`apache-age-pgx-v5` leaves free — both taken per D2
