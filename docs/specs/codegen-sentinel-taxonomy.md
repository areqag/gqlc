...
| `ErrUnimplementedTypeKind` | A property type of a KIND gqlc has built no emission for — today the closed dynamic unions alone, which the schema front end resolves and no backend renders. `RECORD` was the other until stage 1 of `gqlc-x9tg7` gave both backends an emission for it. The one sentinel here carrying a "yet", and the only one that is not a target's answer: the two above it are refusals a backend makes and another backend may not, while this one holds on every target at once, which is why the refusal lives in `prepare.go` and both type maps are untouched by it. Apart from `ErrUnrepresentableWidth` because a table falling off its switch would name the wrong edit — there is no declared width to change, at any width, on any backend. Asked before the carrier question and asked RECURSIVELY through list elements AND record fields, since a shallow check admits `LIST<UNION<…>>` and `RECORD<u UNION<…>>` and lets the union die inside the table as a width error, which is the confusion the sentinel exists to prevent. | ADR 0039, `gqlc-h9n.33` |
...
| `ErrUnimplementedTypeKind` | A schema property whose type is of a kind gqlc emits nothing for, or whose type reaches such a kind through list elements or record fields. Asked BEFORE both questions above it: a union has no carrier on any target, so the table would answer `ErrUnrepresentableWidth` and send the reader to change a width that is not the obstacle. The recursion is what makes `LIST<ANY<INT32\|STRING>>` and `RECORD<u ANY<INT32\|STRING>>` land here rather than there. | Phase Z |
| `ErrRecordFieldCollision` | A schema property whose type is a record with two field names mangling to one Go struct field, or with a field name of underscores alone, at any depth reachable through list elements and record fields. Asked after the kind walk, so a record carrying BOTH an unbuilt kind and a colliding pair is reported as the unbuilt kind — that record has no emission at any spelling, and the field rename would not help. | Phase Z |
...
| `ErrUnimplementedTypeKind` | A projected column whose property type is of a kind gqlc emits nothing for. Assembled, one step past the width row's argument: Phase Z refuses a declared union property, so a column reaching this check is one no declared property backs. | Phase A |
| `ErrUnimplementedTypeKind` | A query parameter whose property type is of a kind gqlc emits nothing for. Assembled, on the same argument. | Phase A |
| `ErrRecordFieldCollision` | A projected column carrying a record whose fields have no legal Go spelling. Assembled, on the kind row's argument. The record is legal as a KIND and illegal only as a SPELLING, which is what carries it past the walk and to this site. | Phase A |
...
| `ErrUnimplementedTypeKind` | A list element whose leaf is a property type of a kind gqlc emits nothing for. Assembled on the width row's argument. Phase B splits a list property into its element before reaching here, so what this site sees is the bare kind; the nesting the walk exists to see through is answered one level up, at whichever of the three sites above declared the list. | Phase A |
