...
| `UUID` | `scopePackage` | every target | every target |
...
Under either neo4j major the over-broad rows are six of the twenty-three: `DBTX`
...
for the same reason. `WithTx` and
...
On `apache-age-pgx-v5` the over-broad rows are two of the twenty-three:
the same `Commit` and `Rollback`, over-broad for the receiver reason
above rather than because the target leaves the name free. `UUID` was a
third while this backend refused the width; it carries it now and
declares `uuid.go` on the trigger the neo4j targets do. The five temporal
...
Six and two count the target axis alone. The batch axis moves
...
five more again on one of the second, and the `UUID` row on one of the
third. Fourteen for a batch of all three shapes under either neo4j major,
ten for an AGE-only one. `NODE TYPE DBTX` is refused
on a name neo4j leaves free — taken per D2
