...
That is not cosmetic, and it is the half of this recipe that was wrong until 2026-08-23 (bd `gqlc-uz3c`). CI's `tidy` job runs `.github/scripts/check-pr-closes.py`, which resolves the PR's bead from a `Bead:` / `Refs:` line in the body, and falls back to a bead id in the **branch** name. The recipe above put the slug in the DIRECTORY and left the branch descriptive, so the fallback never fired: measured over the 60 most recently merged PRs on 2026-08-23, 57 head branches carried no `gqlc-` id. A body that then says `Closes #N` fails the gate, and the failure is close to unreadable — `tidy` gates lint, test and codegen-fence, so those three report `skipping` and the check table looks like a run still in progress. Three PRs in one night each burned about an hour on it and one lane died before diagnosing it. `.githooks/pre-push` now warns (advisorily; it never refuses) when it publishes a branch whose name carries no id.
