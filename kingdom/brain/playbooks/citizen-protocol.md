...
9. File freely, and label what you file. A defect you find while working a
   ...
   **A design is filed as a PAIR, at one price.** When what you file is a
   `class:architect` bead — including when you stop under step 7 because the
   work outgrew its bead — it is filed together with the `class:warrior`
   execution bead that will carry the design out, blocked behind it. That holds EVEN WHEN nobody
