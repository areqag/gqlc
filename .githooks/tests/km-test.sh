...
# other.
wake_seat() { mkdir -p "$KM_STATE_DIR/seats/$1"; echo awake >"$KM_STATE_DIR/seats/$1/status"; }
...
fill_cap() {
    ...
    [ "$n" -eq "$want" ] ||
        bad "fill_cap fills the cap" "wanted $want capped seats awake, the roster offered only $n"
    for s in $CAPPED_WAR; do
        case "$woke" in *" $s "*) ;; *) FREE_SEAT="$s"; break ;; esac
    done
    for s in "$@"; do wake_seat "$s"; done
}
