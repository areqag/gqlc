...
// gate (internal/codegen/age/dialect.go) refuses the constructor names a
// live session was measured on — date() among them — on the query TEXT,
// ahead of the carrier question, because generated code runs that text
// verbatim (ADR 0005) and the pinned AGE 1.7.0 answered date() with
// "function ag_catalog.date(...) does not exist". localtime() has no such
// witness and so is not refused there, which leaves the portable answer
// this fixture names. Give localtime() a witness and this fixture has to
// move with it.
