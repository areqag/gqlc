...
// decodeCompany decodes an agtype vertex into a Company struct, enforcing
// the label and the per-property nullability the schema declares.
func decodeCompany(raw []byte) (Company, error) {
	label, props, err := agtypeEntity(raw, "::vertex")
	if err != nil {
		return Company{}, fmt.Errorf("decode Company: %w", err)
	}
	if label != "Company" {
		return Company{}, fmt.Errorf("decode Company: expected label %q, got %q", "Company", label)
	}
	var out Company
...
// decodeCompanyKnows decodes an agtype edge into a CompanyKnows struct, enforcing
// the label and the per-property nullability the schema declares.
func decodeCompanyKnows(raw []byte) (CompanyKnows, error) {
	label, props, err := agtypeEntity(raw, "::edge")
	if err != nil {
		return CompanyKnows{}, fmt.Errorf("decode CompanyKnows: %w", err)
	}
	if label != "KNOWS" {
		return CompanyKnows{}, fmt.Errorf("decode CompanyKnows: expected label %q, got %q", "KNOWS", label)
	}
...
// decodePersonKnows decodes an agtype edge into a PersonKnows struct, enforcing
// the label and the per-property nullability the schema declares.
func decodePersonKnows(raw []byte) (PersonKnows, error) {
	label, props, err := agtypeEntity(raw, "::edge")
	if err != nil {
		return PersonKnows{}, fmt.Errorf("decode PersonKnows: %w", err)
	}
	if label != "KNOWS" {
		return PersonKnows{}, fmt.Errorf("decode PersonKnows: expected label %q, got %q", "KNOWS", label)
	}
	var out PersonKnows
	value0, err := agtypeProperty(props, "since", agtypeInt64)
	if err != nil {
