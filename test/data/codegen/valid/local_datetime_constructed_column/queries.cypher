...
// AGE defines none of the temporal constructors openCypher spells
// (internal/codegen/age/types.go Temporal, measured live in
// live_age_dialect_test.go), so the backend refuses this column at
// generation. A fixture carrying it can never enrol apache-age-pgx-v5,
// and the widths that fixture does carry are admitted on AGE from
// gqlc-mv3r — so keeping them together would have held the whole round
// trip to the narrowest query in it.
