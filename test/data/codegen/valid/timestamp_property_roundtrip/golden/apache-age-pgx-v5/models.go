...
func decodeEvent(raw []byte) (Event, error) {
	...
	out.Id = value0
	value1, err := agtypeProperty(props, "occurredAt", agtypeInstant)
	if err != nil {
		return Event{}, fmt.Errorf("decode Event.OccurredAt: %w", err)
	}
	out.OccurredAt, err = agtypeZone(props, "occurredAtOffset", value1)
	if err != nil {
		return Event{}, fmt.Errorf("decode Event.OccurredAt: %w", err)
	}
	value2, err := agtypeNullableProperty(props, "seenAt", agtypeInstant)
	if err != nil {
		return Event{}, fmt.Errorf("decode Event.SeenAt: %w", err)
	}
	if value2 != nil {
		zoned, err := agtypeZone(props, "seenAtOffset", *value2)
		if err != nil {
			return Event{}, fmt.Errorf("decode Event.SeenAt: %w", err)
		}
		value2 = &zoned
