...
// decodeBin decodes an agtype vertex into a Bin struct, enforcing
// the label and the per-property nullability the schema declares.
func decodeBin(raw []byte) (Bin, error) {
	label, props, err := agtypeEntity(raw, "::vertex")
	if err != nil {
		return Bin{}, fmt.Errorf("decode Bin: %w", err)
	}
	if label != "Bin" {
		return Bin{}, fmt.Errorf("decode Bin: expected label %q, got %q", "Bin", label)
	}
	var out Bin
	value0, err := agtypeNullableProperty(props, "bag", agtypeListOfAny)
	if err != nil {
		return Bin{}, fmt.Errorf("decode Bin.Bag: %w", err)
	}
	out.Bag = value0
	...
	out.Id = value1
	value2, err := agtypeProperty(props, "loose", agtypeListOfAny)
	if err != nil {
	...
	if err != nil {
		return Bin{}, fmt.Errorf("decode Bin.Piles: %w", err)
	}
	out.Piles = value3
	return out, nil
}
