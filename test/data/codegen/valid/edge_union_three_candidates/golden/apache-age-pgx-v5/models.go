...
// decodePost decodes an agtype vertex into a Post struct, enforcing
// the label and the per-property nullability the schema declares.
func decodePost(raw []byte) (Post, error) {
	...
	if label != "Post" {
		return Post{}, fmt.Errorf("decode Post: expected label %q, got %q", "Post", label)
	}
	var out Post
	value0, err := agtypeProperty(props, "id", agtypeInt64)
	if err != nil {
...
// decodeAUTHORED decodes an agtype edge into a AUTHORED struct, enforcing
// the label and the per-property nullability the schema declares.
func decodeAUTHORED(raw []byte) (AUTHORED, error) {
	label, props, err := agtypeEntity(raw, "::edge")
	if err != nil {
		return AUTHORED{}, fmt.Errorf("decode AUTHORED: %w", err)
	}
	if label != "AUTHORED" {
		return AUTHORED{}, fmt.Errorf("decode AUTHORED: expected label %q, got %q", "AUTHORED", label)
	}
	var out AUTHORED
...
// decodeLIKES decodes an agtype edge into a LIKES struct, enforcing
// the label and the per-property nullability the schema declares.
func decodeLIKES(raw []byte) (LIKES, error) {
	label, _, err := agtypeEntity(raw, "::edge")
	if err != nil {
		return LIKES{}, fmt.Errorf("decode LIKES: %w", err)
	}
	if label != "LIKES" {
		return LIKES{}, fmt.Errorf("decode LIKES: expected label %q, got %q", "LIKES", label)
	}
	var out LIKES
	return out, nil
}

// REPOSTED corresponds to the REPOSTED edge type (Person -> Post).
type REPOSTED struct {
}

func (REPOSTED) isGetActionR() {}

// decodeREPOSTED decodes an agtype edge into a REPOSTED struct, enforcing
// the label and the per-property nullability the schema declares.
func decodeREPOSTED(raw []byte) (REPOSTED, error) {
	label, _, err := agtypeEntity(raw, "::edge")
	if err != nil {
		return REPOSTED{}, fmt.Errorf("decode REPOSTED: %w", err)
	}
	if label != "REPOSTED" {
		return REPOSTED{}, fmt.Errorf("decode REPOSTED: expected label %q, got %q", "REPOSTED", label)
	}
	var out REPOSTED
	return out, nil
}
...
//sumtype:decl
type GetActionR interface{ isGetActionR() }
