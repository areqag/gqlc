...
// decodeWrote decodes an agtype edge into a Wrote struct, enforcing
// the label and the per-property nullability the schema declares.
func decodeWrote(raw []byte) (Wrote, error) {
	label, props, err := agtypeEntity(raw, "::edge")
	if err != nil {
		return Wrote{}, fmt.Errorf("decode Wrote: %w", err)
	}
	if label != "WROTE" {
		return Wrote{}, fmt.Errorf("decode Wrote: expected label %q, got %q", "WROTE", label)
	}
	var out Wrote
	value0, err := agtypeProperty(props, "written", agtypeInt64)
	if err != nil {
