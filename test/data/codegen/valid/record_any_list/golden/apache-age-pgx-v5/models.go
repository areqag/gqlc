...
// Payload corresponds to the Payload node type.
type Payload struct {
	Grid []*[]*map[string]any
	Id   int64
	Rows []*map[string]any
...
// decodePayload decodes an agtype vertex into a Payload struct, enforcing
...
func decodePayload(raw []byte) (Payload, error) {
	...
	if label != "Payload" {
		return Payload{}, fmt.Errorf("decode Payload: expected label %q, got %q", "Payload", label)
	}
	var out Payload
	value0, err := agtypeProperty(props, "grid", agtypeListOfNullableListOfNullableAnyRecord)
	if err != nil {
		return Payload{}, fmt.Errorf("decode Payload.Grid: %w", err)
	}
	out.Grid = value0
	value1, err := agtypeProperty(props, "id", agtypeInt64)
	if err != nil {
		return Payload{}, fmt.Errorf("decode Payload.Id: %w", err)
	}
	out.Id = value1
	value2, err := agtypeProperty(props, "rows", agtypeListOfNullableAnyRecord)
	if err != nil {
		return Payload{}, fmt.Errorf("decode Payload.Rows: %w", err)
	}
	out.Rows = value2
	return out, nil
}
...
// agtypeListOfNullableAnyRecord decodes an agtype list of *map[string]any elements.
func agtypeListOfNullableAnyRecord(raw []byte) ([]*map[string]any, error) {
	return agtypeList(raw, agtypeNullableElem(agtypeMap))
}

// agtypeListOfNullableListOfNullableAnyRecord decodes an agtype list of *[]*map[string]any elements.
func agtypeListOfNullableListOfNullableAnyRecord(raw []byte) ([]*[]*map[string]any, error) {
	return agtypeList(raw, agtypeNullableElem(agtypeListOfNullableAnyRecord))
}
