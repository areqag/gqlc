...
func decodeLedger(raw []byte) (Ledger, error) {
	...
	var out Ledger
	value0, err := agtypeNullableProperty(props, "entry", decodeUnionbd73dd3d)
	if err != nil {
...
// record991891e3 is the carrier for RECORD<opened_on DATE NOT NULL>.
type record991891e3 = struct {
	OpenedOn Date
}
...
// decodeRecord991891e3 checks an agtype map into a record991891e3.
func decodeRecord991891e3(raw []byte) (record991891e3, error) {
	var out record991891e3
	fields, err := agtypeObject(raw)
	if err != nil {
		return out, fmt.Errorf("decode RECORD<opened_on DATE NOT NULL>: %w", err)
	}
	value0, err := agtypeRecordField(fields, "opened_on", agtypeDate)
	if err != nil {
		return out, fmt.Errorf("decode RECORD<opened_on DATE NOT NULL>: field %q: %w", "opened_on", err)
	}
	if value0 == nil {
		return out, fmt.Errorf("decode RECORD<opened_on DATE NOT NULL>: field %q is not null but has no value", "opened_on")
	}
	out.OpenedOn = *value0
	return out, nil
}

// encodeRecord991891e3 widens a record991891e3 into the map its agtype text is rendered from.
func encodeRecord991891e3(v record991891e3) (map[string]any, error) {
	out := make(map[string]any, 1)
	value0, err := agtypeDateText(v.OpenedOn)
	if err != nil {
		return nil, fmt.Errorf("encode RECORD<opened_on DATE NOT NULL>: field %q: %w", "opened_on", err)
	}
	out["opened_on"] = value0
	return out, nil
}

// decodeUnionbd73dd3d dispatches an agtype value onto the member of
// UNION<INT64|RECORD<opened_on DATE NOT NULL>> whose wire family it arrived as, and narrows it
// to that member's declared width.
func decodeUnionbd73dd3d(raw []byte) (any, error) {
	body := bytes.TrimSpace(raw)
	if len(body) == 0 {
		return nil, fmt.Errorf("decode UNION<INT64|RECORD<opened_on DATE NOT NULL>>: %q is not an agtype value", raw)
	}
	switch body[0] {
	case '{':
		out, err := decodeRecord991891e3(body)
		if err != nil {
			return nil, fmt.Errorf("decode UNION<INT64|RECORD<opened_on DATE NOT NULL>>: %w", err)
		}
		return out, nil
	}
	...
	return nil, fmt.Errorf("decode UNION<INT64|RECORD<opened_on DATE NOT NULL>>: %q is no member's wire shape", raw)
}

// encodeUnionbd73dd3d validates a value against the member set UNION<INT64|RECORD<opened_on DATE NOT NULL>>
...
func encodeUnionbd73dd3d(v any) (any, error) {
	switch t := v.(type) {
	case int64:
		return t, nil
	case struct {
		...
		out, err := encodeRecord991891e3(t)
		if err != nil {
			return nil, fmt.Errorf("encode UNION<INT64|RECORD<opened_on DATE NOT NULL>>: %w", err)
		}
		return out, nil
	}
	return nil, fmt.Errorf("encode UNION<INT64|RECORD<opened_on DATE NOT NULL>>: no member carries %T", v)
}
