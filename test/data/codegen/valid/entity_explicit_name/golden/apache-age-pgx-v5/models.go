...
// decodeActor decodes an agtype vertex into a Actor struct, enforcing
...
func decodeActor(raw []byte) (Actor, error) {
	...
	if err != nil {
		return Actor{}, fmt.Errorf("decode Actor: %w", err)
	}
	if label != "Person" {
		return Actor{}, fmt.Errorf("decode Actor: expected label %q, got %q", "Person", label)
	}
	var out Actor
	...
	out.Id = value0
	value1, err := agtypeProperty(props, "name", agtypeString)
	if err != nil {
