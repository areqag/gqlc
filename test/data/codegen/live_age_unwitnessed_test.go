...
// which it now does. That allowlist is a name list rather than a pattern, and
// TestEveryLiveTestIsRunByARecipeThatNamesIt (internal/liverecipes) refuses a
// live test missing from it — this file was written before that gate landed and
// measured as unselected under the old recipe.
//
// gqlc-osf1 stays open regardless of enrolment: the bead is done when the
// answers have been READ off a run and written into the gap table, and this
// file only obtains them.
//
