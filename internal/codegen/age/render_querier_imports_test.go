...
// querierImportCases is the table. Apache AGE carries two optional carriers
// rather than neo4j's three — it decodes agtype and names no driver type in
// a signature — so the subsets are four rather than eight.
//
// This backend is where the composition bug actually was. Before bd
// gqlc-1a5 the import block was a switch over combinations, which needs a
// case per subset; adding `iter` to a switch written for `time` alone drops
// `time` from every batch naming both, and nothing in this package compiled
// the result.
...
func TestQuerierImportsMatchTheSignatures(t *testing.T) {
	for _, c := range querierImportCases() {
		t.Run(c.desc, func(t *testing.T) {
			src := string(age.RenderQuerier("q", c.prepared))
...
// name. `context` needs no special case: every method signature spells
// `ctx context.Context` and the regex finds it there like any other.
