...
// pairwise-distinct wire shapes carries `any`, and the member list is
// enforced by generated validation rather than by the type.
//
// The temporal rows are the ones this backend does not share with AGE, and
// they are the point of testing them here. Every temporal width arrives as
// its own dbtype — DATE as dbtype.Date, TIME as dbtype.Time — so DATE and
// STRING are two families here while they are one on AGE, and a zoned
// member rides a union here while AGE refuses it on the container rule.
// That disagreement is what makes AGE's refusal contingent and so obliges
// it to name itself (ADR 0035); this package cannot see the other backend,
// so the disagreement itself is asserted from the composition root.
func TestTypeMapAdmitsAWireDistinctUnion(t *testing.T) {
	admitted := []graph.PropertyType{
		...
		// driver needs no sidecar and the container rule does not bite.
		graph.UnionOf([]graph.UnionMember{{Type: graph.TypeTime}, {Type: graph.TypeString}}),
		// BYTES is the one width the driver hands back as a Go slice of its
		// own, so it is a family apart from every other list.
		graph.UnionOf([]graph.UnionMember{{Type: graph.TypeBytes}, {Type: graph.ListOf(graph.TypeInt32, false)}}),
		// int / float / string / list / map, five families at once.
		graph.UnionOf([]graph.UnionMember{
			{Type: graph.TypeInt32},
			{Type: graph.TypeFloat32},
...
// widths into. A refusal is ok=false, which the preparation sites route to
// ErrUnrepresentableWidth naming the union.
//
// The first row is spec §8's falsifier: UNION<INT32|INT64> must be refused
// on BOTH backends, every integer width widening to int64 everywhere.
// Refusing rather than admitting is the whole content of the closed union
// over ADR 0020's open one — a decoder handed an int64 cannot say which
// declared member it was, so gqlc will not pretend to narrow it.
func TestTypeMapRefusesAUnionWhoseMembersShareAWireFamily(t *testing.T) {
	refused := map[string]graph.PropertyType{
		"integer widths all widen to int64": graph.UnionOf([]graph.UnionMember{
			{Type: graph.TypeInt32}, {Type: graph.TypeInt64},
		}),
		"a signed and an unsigned width widen to the same int64": graph.UnionOf([]graph.UnionMember{
		...
		"a bare LIST is the same []any a declared one is": graph.UnionOf([]graph.UnionMember{
			{Type: graph.TypeList}, {Type: graph.ListOf(graph.TypeString, false)},
		}),
		"two declared records both arrive as map[string]any": graph.UnionOf([]graph.UnionMember{
			{Type: unionAddr},
		...
		"the fieldless record is still a map on the wire": graph.UnionOf([]graph.UnionMember{
			{Type: graph.RecordOf(nil)}, {Type: graph.TypeAnyRecord},
		}),
...
//
// It matters because the two refusals owe different messages and different
// repairs: a carrier refusal names a width to change, and an admission
// refusal names a PAIR, either of which the author may drop.
...
func TestAUnionRidesAContainer(t *testing.T) {
	...
	t.Run("a list of an admitted union carries its element", func(t *testing.T) {
		got, ok := neo4j.TypeMap{}.Property(graph.ListOf(distinct, true))
		require.True(t, ok)
	...
	t.Run("a list of a refused union is refused", func(t *testing.T) {
		_, ok := neo4j.TypeMap{}.Property(graph.ListOf(collides, false))
		require.False(t, ok)
	})

	t.Run("a record field of an admitted union carries it", func(t *testing.T) {
		got, ok := neo4j.TypeMap{}.Property(graph.RecordOf([]graph.RecordField{
	...
	t.Run("a record field of a refused union refuses the record", func(t *testing.T) {
		_, ok := neo4j.TypeMap{}.Property(graph.RecordOf([]graph.RecordField{
