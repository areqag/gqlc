...
// TestARecordUUIDFieldRidesTheStringWire holds the record path to ADR
// 0047 on both majors: a UUID field decodes from a string through the
// checked toUUID, encodes through fromUUID, and names no driver type.
//
...
//
// The dbtype rows are the ones with a failure behind them. The record
// file's import walk once counted UUID among the carriers that assert a
// dbtype value on decode; a walk that still did would import a package
// nothing in the file names, which does not compile.
func TestARecordUUIDFieldRidesTheStringWire(t *testing.T) {
	rec := graph.RecordOf([]graph.RecordField{
	...
	for name, target := range map[string]neo4j.DriverTarget{"v5": neo4j.TargetV5, "v6": neo4j.TargetV6} {
		t.Run(name, func(t *testing.T) {
			...
			require.True(t, ok, "the premise: this major's table carries a record whose field is a UUID")
			...
			assert.Contains(t, out, "fromUUID(",
				"the encode direction renders the field as the text it is stored as")
			assert.Contains(t, out, ", err := toUUID(",
				"the decode direction parses a string and can fail, so it is a checked call and not a bare conversion")
			assert.NotContains(t, out, "dbtype",
				"a UUID reaches no driver type in either direction, and an import nothing names does not compile")
		})
	}
}
