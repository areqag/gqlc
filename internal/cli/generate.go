...
func runGenerate(cmd *cobra.Command, cfgPath string) error {
	...
	// Stage-7 accumulation: nil error + populated Diagnostics. Files is
	// nil in this branch; nothing is written.
	if len(res.Diagnostics) > 0 {
		return reportDiagnostics(cmd, res.Diagnostics)
	}
...
// reportDiagnostics prints every diagnostic line to stderr in pipeline
// order, then returns the singular/plural summary (spec §2.3).
func reportDiagnostics(cmd *cobra.Command, diags []string) error {
	for _, d := range diags {
...
func inspectOutput(dir string) (targetPlan, error) {
	...
	// Step 3.
	offenders, wipe, err := sweepOutputEntries(dir, entries)
	if err != nil {
		return targetPlan{}, fmt.Errorf("output: %w", err)
	}
...
// sweepOutputEntries is §7.1 step 3 — sweep every entry, collecting every
// offender, not the first (ADR 0012: the abort names them all). A read
// failure aborts: the directory's state cannot be proven, so nothing is
// touched.
func sweepOutputEntries(dir string, entries []fs.DirEntry) (offenders, wipe []string, _ error) {
