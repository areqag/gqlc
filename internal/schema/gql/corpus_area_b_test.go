...
var corpusAreaB = []corpusEntry{
	...
	},
	{
		...
		feature:  "mandatory",
		bead:     "gqlc-0ri",
		reason:   "property types with no label set — the schema model keys node types by label set (the `Nodes: make(map[graph.LabelSetKey]schema.NodeType)` construction in func (rawSchema) resolve, resolve.go), and an empty key is not a blank name but the same key for every unlabelled type, so a second one would collide rather than differ. Decided in ADR 0018: declined on gqlc's model, not on a reading of the standard. nodeTypeImpliedContent alternative 2 has no other route",
	},
	{
	...
	},
	{
		...
		feature:  "mandatory",
		bead:     "gqlc-0ri",
		reason:   "the value type is incidental — constructed_path.gql already pins that PATH declines. This file pins that the phrase-form node collector forwards the decline at all: every value-type decline in the corpus is written as a pattern-form node, so dropping the l.fail from EnterNodeTypePhrase's error arm left the suite green, silently discarding a phrase-form node that carries an unsupported property type. It spelled RECORD until gqlc-h9n.33 made records resolve, at which point the file would have pinned nothing; the family moved to a permanently declined one so the guard survives the next lifted decline too",
	},
	{
	...
	},
	{
		...
		outcome:  unsupported,
		sentinel: gql.ErrDuplicatePropertyName,
		feature:  "mandatory",
		bead:     "gqlc-tlbo",
