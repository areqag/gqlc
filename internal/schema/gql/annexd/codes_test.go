...
func TestCodesSortedAndUnique(t *testing.T) {
	require.True(t, sort.StringsAreSorted(Codes), "annexd.Codes is not sorted")
...
func TestCodesSnapshotCardinality(t *testing.T) {
	require.Len(t, Codes, 228, "annexd.Codes cardinality has drifted from the 2026-07-26 snapshot; update SOURCE.md if the regeneration was intentional")
}
...
func TestHasKnownRealCode(t *testing.T) {
	for _, c := range []string{"GG02", "GE03", "GH02"} {
		require.True(t, Has(c), "annexd.Has(%q) is false; the vendored snapshot is missing a code the earlier corpus already cited", c)
	}
}
...
func TestHasRejectsFabricated(t *testing.T) {
	for _, c := range []string{"GG99", "G001", "GX99", "gg02", "GG02 ", ""} {
		require.False(t, Has(c), "annexd.Has(%q) is true; a value outside the vendored snapshot must be rejected", c)
	}
}
