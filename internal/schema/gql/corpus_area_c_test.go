...
var corpusAreaC = []corpusEntry{
	...
	},
	{
		...
		outcome:  unsupported,
		sentinel: gql.ErrImpliedLabelIsKeyLabel,
		feature:  "GG21",
		bead:     "gqlc-0ri",
	...
	},
	{
		...
		outcome:  unsupported,
		sentinel: gql.ErrEndpointFillerImpliesLabels,
		feature:  "GG21",
		bead:     "gqlc-0ri",
	...
	},
	{
		...
		feature:  "mandatory",
		bead:     "gqlc-0ri",
		reason:   "no corpus file put an unsupported property type on an edge at all, so edgeContent's error arm never returned a non-nil error across the whole suite; deleting the propagation from edgeContent and from EnterEdgeTypePattern's arm both left the tree green, silently dropping the edge and the relationship with it. The value type is incidental — constructed_path.gql pins that PATH declines on a node. It spelled RECORD until gqlc-h9n.33 made records resolve; a resolving value type discharges nothing here, so the family moved rather than the entry",
	},
	{
		...
		feature:  "mandatory",
		bead:     "gqlc-0ri",
		reason:   "the phrase spelling of pattern_unsupported_property_type.gql. The two edge collectors forward edgeContent's error through separate arms, so the pattern file grounds only its own; dropping the l.fail from EnterEdgeTypePhrase's arm left the suite green even with the pattern-form file present. It moved off RECORD with its pattern-form twin when gqlc-h9n.33 made records resolve",
	},
	{
		file:     "18.3-edge-type/endpoint_no_filler.gql",
		outcome:  unsupported,
		sentinel: gql.ErrUnknownEndpoint,
		feature:  "mandatory",
		bead:     "gqlc-0ri",
